...
def build_role_dependency_graph(role):
    technical_direction = getattr(role, "technical_direction", None)
    role_value = _role_value_with_member_count(role)

    if technical_direction:
        direction_label = technical_direction.name
        direction_url = _reverse_or_none("users:technicaldirection_change", getattr(technical_direction, "pk", None))
    elif getattr(role, "is_admin_role", False):
        direction_label = "Transverse"
        direction_url = None
    else:
        direction_label = None
    ...
    nodes = [
        _graph_node(
            ...
            "Rôle",
            value=role_value,
            url=_reverse_or_none("users:role_change", getattr(role, "pk", None)),
            layout={"x": 20, "y": 90},
        ),
        _graph_node(
            "technical_direction",
    ...
    links = [
        {"from": "role", "to": "technical_direction", "route": "horizontal"},
    ]
